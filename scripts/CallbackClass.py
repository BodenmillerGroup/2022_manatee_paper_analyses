class CallbackClass:
    ...
    def append_all_tasks_to(self, listname, key_part, log_dict, labels):
        task_vals = [
            value for key, value in log_dict.items() if key.startswith(key_part)
        ]
    ...
    def __call__(self, log_dict):
        for strat in self.strategy:
            self.append_to(
                getattr(self, f"solution_{strat}"), f"Solution/{strat}", log_dict
            )
            self.append_to(
                getattr(self, f"lscale_{strat}"), f"Lengthscale/{strat}", log_dict
            )
            ...
            self.append_all_tasks_to(
                getattr(self, f"inclusion_{strat}"),
                f"Inclusion probability/{strat}",
                log_dict,
                getattr(self, f"labels_{strat}"),
            )
            self.append_all_tasks_to(
                getattr(self, f"objagree_{strat}"),
                f"Correlation/{strat}",
                log_dict,
                getattr(self, f"labels_{strat}"),
            )
            self.append_all_tasks_to(
                getattr(self, f"noise_{strat}"),
                f"Noise/{strat}",
                log_dict,
                getattr(self, f"labels_{strat}"),
            )
            self.append_all_tasks_to(
                getattr(self, f"max_{strat}"),
                f"Max/{strat}",
                log_dict,
                getattr(self, f"labels_{strat}"),
            )
