...
# This function sets logging settings (wandb or CallbackClass to be run in notebook or
# saved to disk) and calls `main()`.
def log_settings(
    experiment,
    logging,
    strategy,
    ...
    if experiment == "imc":
        if crossval:
            ...
            for fold in range(5):
                # Split
                data1_train, data2_train, data1_test, data2_test = imc_setup.split_data(
                    data1, data2
                )
        ...
        if crossval:
            ...
            for fold in range(5):
                ...
                (
                    ...
                    data1_test,
                    data2_test,
                ) = citeseq_setup.split_data(data1, data2)
                mobo_output_dict, success = main_crossval(
                    data1_train,
                    data2_train,
                    data1_test,
...
def main_crossval(
    ...
    if experiment == "toy":
        ...
        train_y, ari, nmi = imc_setup.true_f(
...
def set_mobo_params(num_tasks, ucb_scal):
    ...
    acq_params = []
    acq_params.append(torch.tensor([1.0]).repeat(num_tasks))
    acq_params.append(1.0)
...
def run_experiment(
    ...
    if strategy == "manatee" or strategy == "random prob":
        if strategy == "random prob" and ablate != "none":
            assert (
                False
            ), f"Ablating behaviours is only possible with manatee, not with {strategy}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--experiment", type=str, help="Type of experiment. Options: toy, imc, citeseq"
    )
    parser.add_argument("--logging", type=str, default="nolog", help="wandb / nolog")
    parser.add_argument(
        "--strategy",
        nargs="+",
        default=["manatee"],
        help="Strategy. "
        "Options: manatee, random prob, random loc, botorch, qparego, usemo",
    )
    parser.add_argument("--x_min", type=float, help="Min range of x")
    parser.add_argument("--x_max", type=float, help="Max range of x")
    parser.add_argument(
        "--num_train_pts", type=int, default=5, help="Training set size"
    )
    parser.add_argument(
        "--optimise_iter", type=int, default=10, help="Number of BO acquisition steps"
    )
    parser.add_argument("--plot", type=bool, default=False, help="Plot or not")
    parser.add_argument("--seed", default=10, help="Set seed")
    parser.add_argument("--desc", default="default", help="Experiment description")
    parser.add_argument("--crossval", default=False, help="Run cross-val?")
    parser.add_argument(
        "--ablate",
        type=str,
        default="none",
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        help="Compute the AS acquisition function E[UCB(scal)] "
        "with MC sampling (mc) or exhaustively (exhaustive)?",
    )
