...
def get_ucb_beta(t):
    beta = 0.125 * torch.tensor(math.log(2 * t + 1))
...
def mc_sample_std(f_pred, p_lambdas, S):
    ...
    for s in range(S):
        ...
        pair_indices = [
            [covar_indices[0][i].item(), covar_indices[1][i].item()]
            for i in range(len(covar_indices[0]))
        ]
        lambda_matrix = torch.zeros(num_tasks, num_tasks)
        lambda_pairs = torch.tensor(
            [torch.prod(lambda_sample[j]).item() for j in pair_indices]
        )
        lambda_matrix[covar_indices[0], covar_indices[1]] = lambda_pairs
        assert (
            lambda_matrix
            == torch.triu(
                lambda_sample.view(1, -1) * lambda_sample.view(-1, 1), diagonal=1
            )
        ).all(), "didn't pass assert"
        covar_term = torch.sum(
            lambda_matrix * torch.triu(f_pred.covariance_matrix, diagonal=1)
        )
...
def exhaustive_lambda_vector_probs(p_lambdas):
    ...
    all_lambda_vectors = torch.cartesian_prod(
        *[torch.tensor([0.0, 1.0]) for i in range(num_tasks)]
    )
    one_minus_p_lambdas = 1.0 - p_lambdas
    arr1 = all_lambda_vectors * p_lambdas.view(1, num_tasks)
    arr2 = (1.0 - all_lambda_vectors) * one_minus_p_lambdas.view(1, num_tasks)
...
def exhaustive_std(f_pred, all_lambda_vectors, all_lambda_vector_probs):
    ...
    for i, vec in enumerate(all_lambda_vectors):
        ...
        covar_indices = torch.triu_indices(num_tasks, num_tasks, offset=1)
        pair_indices = [
            [covar_indices[0][k].item(), covar_indices[1][k].item()]
            for k in range(len(covar_indices[0]))
        ]
        lambda_matrix = torch.zeros(num_tasks, num_tasks)
        lambda_pairs = torch.tensor([torch.prod(vec[j]).item() for j in pair_indices])
        lambda_matrix[covar_indices[0], covar_indices[1]] = lambda_pairs
        assert (
            lambda_matrix == torch.triu(vec.view(1, -1) * vec.view(-1, 1), diagonal=1)
        ).all(), "didn't pass assert"
        covar_term = torch.sum(
            lambda_matrix * torch.triu(f_pred.covariance_matrix, diagonal=1)
        )
        ...
        assert (
            sum_term > -1e-4
        ), f"covariance estimate in acq function is negative and too large: {sum_term}"
...
def noise_include_no_BF_match(noise):
    noise[noise >= 1.0] = 1.0 - 1e-3
    return 2.0 - 2.0 * noise
...
def objagree_exclude_no_BF_match(eta):
    eta[eta == 1.0] = 1.0 - 1e-3
    return 2.0 - 2.0 * eta


def max_include_bernoulli(p_1=torch.tensor([0.75])):
    assert p_1 >= 0.0 and p_1 <= 1.0, f"p_1 should be in [0,1], not {p_1}"
    return Bernoulli(p_1)


def max_exclude_bernoulli(p_0=torch.tensor([0.25])):
    assert p_0 >= 0.0 and p_0 <= 1.0, f"p_0 should be in [0,1], not {p_0}"
...
def logp_include_no_BF_match(obs_noises, corr_means, is_max, ablate):
    ...
    logp_B_include = (
        lambda_prior().log_prob(torch.tensor(1.0))
    ...
    logp_B_exclude = (
        lambda_prior().log_prob(torch.tensor(0.0))
    ...
    if ablate == "cor":
        logp_B_include = logp_B_include - torch.log(
            objagree_include_no_BF_match(corr_means)
        )
        logp_B_exclude = logp_B_exclude - torch.log(
            objagree_exclude_no_BF_match(corr_means)
        )

    elif ablate == "noise":
        logp_B_include = logp_B_include - torch.log(
            noise_include_no_BF_match(obs_noises)
        )
        logp_B_exclude = logp_B_exclude - torch.log(
            noise_exclude_no_BF_match(obs_noises)
        )
        ...
        assert False, (
            f"{ablate} is not a valid behaviour to ablate. "
            "Possible options: cor, noise, max, none"
        )
    ...
    log_noise_lambda1 = lambda_prior().log_prob(torch.tensor(1.0)) + torch.log(
        noise_include_no_BF_match(obs_noises)
    )

    log_corr_lambda1 = lambda_prior().log_prob(torch.tensor(1.0)) + torch.log(
        objagree_include_no_BF_match(corr_means)
    )

    log_max_lambda1 = lambda_prior().log_prob(
        torch.tensor(1.0)
    ) + max_include_bernoulli().log_prob(is_max)

    log_noise_lambda0 = lambda_prior().log_prob(torch.tensor(0.0)) + torch.log(
        noise_exclude_no_BF_match(obs_noises)
    )

    log_corr_lambda0 = lambda_prior().log_prob(torch.tensor(0.0)) + torch.log(
        objagree_exclude_no_BF_match(corr_means)
    )

    log_max_lambda0 = lambda_prior().log_prob(
        torch.tensor(0.0)
    ) + max_exclude_bernoulli().log_prob(is_max)
...
def optimise_acq(optimizer, acq_f, x_probe, model, acq_params, scheduler):
    if acq_f == mobo_ucb_scalarized:
        x_probe, losses_acq = optimise_acq_lbfgs(
            optimizer, acq_f, x_probe, model, acq_params, scheduler
        )
    if acq_f == mobo_ucb_scalarized_samples:
        x_probe, losses_acq = optimise_acq_adam(
            optimizer, acq_f, x_probe, model, acq_params, scheduler
        )
    if acq_f == mobo_acq:
        x_probe, losses_acq = optimise_acq_lbfgs(
...
def optimise_acq_lbfgs(
    optimizer, acq_f, x_probe, model, acq_params, scheduler, steps=5
):
    losses_all = []
...
def optimise_acq_adam(
    optimizer, acq_f, x_probe, model, acq_params, scheduler, steps=300
):
    losses = []
    assert (
        scheduler.T_max == steps
    ), f"number of steps for scheduler should equal {steps}, not {scheduler.T_max}"
