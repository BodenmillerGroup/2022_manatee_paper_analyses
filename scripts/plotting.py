...
def plot_gp(
    train_x, train_y, test_x, f_pred, observed_pred, title=None, new_point_idx=None
):
    ...
    if len(train_y.shape) > 1:
        ...
        for task in range(num_tasks):
            ...
            if new_point_idx is not None:
                plt.plot(
                    train_x.numpy()[new_point_idx, 0],
                    train_y.numpy()[new_point_idx, task],
                    "r*",
                    label="New observation",
                )
            plt.plot(
                test_x[:, 0].numpy(),
                f_pred.mean[:, task].numpy(),
                color="blue",
                label="Mean",
            )
            plt.fill_between(
            ...
            plt.fill_between(
                test_x[:, 0],
                (
                    f_pred.mean[:, task] - 2 * torch.sqrt(f_pred.variance[:, task])
                ).numpy(),
                (
                    f_pred.mean[:, task] + 2 * torch.sqrt(f_pred.variance[:, task])
                ).numpy(),
        ...
        if new_point_idx is not None:
            plt.plot(
                train_x.numpy()[new_point_idx],
                train_y.numpy()[new_point_idx],
                "r*",
                label="New observation",
            )
...
def plot_gp_sampled(
    train_x, train_y, num_orig_train_pts, test_x, f_pred, observed_pred, title=None
):
    ...
    if len(train_y.shape) > 1:
        ...
        for task in range(num_tasks):
            plt.figure(figsize=(5, 3))
            plt.plot(
                train_x[:num_orig_train_pts, 0].numpy(),
                train_y[:num_orig_train_pts, task].numpy(),
                "k*",
                label="Observations",
            )

            # If only one pt has been sampled so far, plot it in different colour as
            # current
            num_sampled = train_x.shape[0] - num_orig_train_pts
            if num_sampled == 1:
                plt.plot(
                    train_x[-1, 0].numpy(),
                    train_y[-1, task].numpy(),
                    color="magenta",
                    marker="*",
                    label="Current",
                )
            # Otherwise, go through the list of sampled points and plot last one as
            # current
            else:
                for pt in range(num_sampled):
                    if pt == 0:
                        plt.plot(
                            train_x[num_orig_train_pts + pt, 0].numpy(),
                            train_y[num_orig_train_pts + pt, task].numpy(),
                            "r*",
                            label="Sampled",
                        )
                    if pt == num_sampled - 1:
