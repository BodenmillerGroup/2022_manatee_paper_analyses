...
def bayes_opt(
    ...
    for j in range(optimise_iter):
        ...
        with torch.no_grad():
            ...
            if strategy == "manatee":
                ...
                for task in range(num_tasks):
                    log_dict[f"Noise/{strategy}/{log_titles[task]}"] = task_obs_noises[
                        task
                    ].item()
                ...
                for task in range(num_tasks):
                    log_dict[f"Correlation/{strategy}/{log_titles[task]}"] = corr_means[
                        task
                    ].item()
            ...
            for task in range(num_tasks):
                log_dict[
                    f"Inclusion probability/{wandb_plot_title}/{log_titles[task]}"
                ] = lambda_probs[task].item()
            if strategy == "manatee":
                for task in range(num_tasks):
                    log_dict[
                        "Partial inclusion probability/"
                        f"{strategy}/noise/{log_titles[task]}"
                    ] = partial_lambda[task, 0].item()
                    log_dict[
                        "Partial inclusion probability/"
                        f"{strategy}/correlation/{log_titles[task]}"
                    ] = partial_lambda[task, 1].item()
                    log_dict[
                        "Partial inclusion probability/"
                        f"{strategy}/max/{log_titles[task]}"
                    ] = partial_lambda[task, 2].item()
        ...
        if acq_fun == acq.mobo_ucb_scalarized:
            (
                all_lambda_vectors,
                all_lambda_vector_probs,
            ) = acq.exhaustive_lambda_vector_probs(lambda_probs)
            if j == 0:
...
def bayes_opt_botorch(
    ...
    for j in range(optimise_iter):
        ...
        # Init botorch model
        mll_qnehvi, model_qnehvi = gp.initialize_botorch_model(
            train_x_qnehvi, train_obj_qnehvi, standard_bounds
        )
        ...
        try:
            ...
            qnehvi_sampler = SobolQMCNormalSampler(
                num_samples=MC_SAMPLES, batch_range=batch_range
            )
...
def bayes_opt_qparego(
    ...
    for j in range(optimise_iter):
        ...
        # Init botorch model
        mll_qparego, model_qparego = gp.initialize_botorch_model(
            train_x_qparego, train_obj_qparego, standard_bounds
        )
        print("Initialised model.")
        ...
        try:
            ...
            # define the qEI and qNEI acquisition modules using a QMC sampler
            batch_range = (0, -1)
            qparego_sampler = SobolQMCNormalSampler(
                num_samples=MC_SAMPLES, batch_range=batch_range
            )
...
def bayes_opt_usemo(
    ...
    for j in range(optimise_iter):
        ...
        for i in range(len(cheap_pareto_set)):
            if not any((cheap_pareto_set[i] == x).all() for x in GPs[0].xValues):
